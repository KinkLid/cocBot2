...
Если в удаленном репозитории есть изменения, выполните на сервере от root через `sudo`:

```bash
sudo /opt/cocbot/scripts/update_from_git.sh \
  /opt/cocbot
```

Скрипт от root делает `git fetch`, затем принудительно синхронизирует рабочую копию с `origin/<текущая_ветка>` через `git reset --hard`, после чего запускает полный server-side deploy (`venv`, зависимости, миграции, systemd restart). Сам systemd-сервис продолжает запускать приложение от пользователя `cocbot`.

> Важно: `git reset --hard` удалит изменения только tracked-файлов. Runtime-файлы и каталоги `.env`, `config.yaml`, `data/`, `logs/`, `exports/` и `backups/` не удаляются; `git clean` не используется.
...
Для приватного репозитория root на новом сервере должен иметь read-only доступ к `origin`; предпочтителен read-only deploy key. До миграции на новом сервере должна успешно выполняться команда:

```bash
sudo git -c safe.directory=/opt/cocbot \
  -C /opt/cocbot \
  ls-remote --exit-code origin
```

На старом сервере запустите одну команду переноса пользователем, который имеет доступ к `origin` старого clone и является root либо имеет passwordless sudo для `systemctl`:
...
В этом режиме удалённый `/opt/cocbot` обязан быть Git-репозиторием. Git-команды на новом сервере выполняются от root с `safe.directory`, скрипт переключает clone на ту же ветку и тот же commit, что и старый сервер, переносит runtime-файлы и базу, но сохраняет `.git` на новом сервере. После переноса обновления выполняются так:
...
- Git и deploy при обновлении выполняются от root;
- systemd запускает приложение от `cocbot`;
- `.env`, `config.yaml`, `data/`, `logs/`, `exports/` и `backups/` не удаляются;
- `git reset --hard` удалит изменения только tracked-файлов;
- `git clean` не используется;
