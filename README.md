...
- root-доступ по SSH или пользователь с passwordless sudo;
- новый Clash API token, выпущенный для публичного IPv4 нового сервера.

Перед реальным переносом можно проверить доступы и план без изменений на серверах:

```bash
./scripts/migrate_to_new_server.sh --dry-run root@NEW_IP
```
