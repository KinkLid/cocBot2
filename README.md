...
Запускайте перенос на старом сервере из каталога проекта. Основная инструкция состоит из одной команды:

```bash
./scripts/migrate_to_new_server.sh
```

Скрипт сам спросит SSH-адрес нового сервера, каталог проекта и новый `CLASH_API_TOKEN` для IP нового сервера. Потребуется:

- запускать команду на старом сервере;
- локальный пользователь должен быть `root` или иметь passwordless sudo для `systemctl`;
- SSH-доступ к новому серверу;
- SSH-пользователь нового сервера должен быть `root` или иметь remote passwordless sudo;
- новый Clash API token, выпущенный для публичного IPv4 нового сервера.
