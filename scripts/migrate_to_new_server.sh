...
log "Python для локальных проверок: ${PYTHON_BIN}"
if ! "${PYTHON_BIN}" - "${PROJECT_ROOT}" <<'PY'
import os
import sys
...
PY
then
  die_before_cutover $'Не удалось импортировать зависимости проекта выбранным Python.\nПроверьте .venv или установку зависимостей проекта.'
fi
DB_PATH="$("${PYTHON_BIN}" - "${PROJECT_ROOT}" <<'PY'
import os, sys
from pathlib import Path
